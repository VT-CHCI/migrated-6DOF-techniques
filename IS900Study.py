...
#Links Ray to Head
rayTrackingLink = viz.link(head, ray, enabled=True)
rayTrans = rayTrackingLink.preTrans([0,0,.1])
rayRot = rayTrackingLink.preEuler([0,0,0])

#Links Body to Head
bodyLink = viz.link(head, body, enabled=False)
bodyLink.setDstFlag(viz.LINK_POS_RAW)
bodyLink.preTrans([0,-1.2,0])

#Links WIM to Head
wimTrackingLink = viz.link(head, miniWorld, enabled=False)
wimTrans = wimTrackingLink.preTrans([0,0,.1])
wimRot = wimTrackingLink.preEuler([0,0,0])
