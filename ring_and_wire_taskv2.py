...
ring = viz.add('models/bounding_box.obj')
...
ring.collideBox(node='Cube.001_Cube.002')
ring.collideBox(node='Cube.002_Cube.003')
ring.collideBox(node='Cube.003_Cube.004')
ring.collideBox(node='Cube.004_Cube.005')
ring.collideBox(node='Cube.005_Cube.006')
ring.collideBox(node='Cube.006_Cube.007')
ring.collideBox(node='Cube.007_Cube.008')
ring.collideBox(node='Cube_Cube.001')
...
spring = ring.addSpring(viz.LINK_POS,linearKd=2,linearKs=200)
...
def updateScene():
	...
	spring.setQuat(vetools.configQuat(data[3:7]))
	...
	if attached:
		...
		spring.setQuat(box.getQuat(viz.ABS_GLOBAL))
		...
		spring.setPosition(box.getPosition(viz.ABS_GLOBAL))
...
def onCollide(e):
	...
	if (not attached) and (e.obj1 != ray) and (e.obj2 != ray):
		print e.obj1,e.obj2
