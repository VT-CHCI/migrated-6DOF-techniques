...
viz.link(ray, objParent, enabled=True)
...
def checkDist(pt1,pt2):
	...
	if not attached: 
		if moveDist < 1.25:
			...
			ray.setScale([.5,.5,.5])
			rayLine.setScale([1,1,.0055])
			rayEnd.setPosition([0,0,.45])
...
def updateScene():
	...
	wimTransOperator.setPosition(vetools.configPos(data2[0:3],calibrationPos,calibration,[0,.5,.5]))
	wimRotateOperator.setQuat(vetools.configQuat(data2[3:7],[0,90,0]))
	
	if wimSelect & attached:
		miniObjParent.setMatrix(objParent.getMatrix(viz.ABS_GLOBAL),viz.ABS_GLOBAL)
		
		XMoveCurrent = miniObjParent.getMatrix()
		XMoveCurrent.preMult(XMoveInit.inverse())
		XMoveCurrent.preMult(XOffset)
		
		obj[0].object.setMatrix(XMoveCurrent.get())
		...
		if button[1]:
			...
			if intersectedObjects:
				...
				if wimSelect:
					...
					XMoveInit = miniObjParent.getMatrix()
					
					XMoveCurrent = miniObjParent.getMatrix()
					XMoveCurrent.preMult(XMoveInit.inverse())
					
					XOffset = intersectedObjects[0].object.getMatrix()
					XOffset.preMult(XMoveCurrent.inverse())
				else:
